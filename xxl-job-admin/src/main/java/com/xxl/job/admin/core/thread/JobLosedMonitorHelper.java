			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						if (!toStop) {
							logger.error(">>>>>>>>>>> sc-job, job fail monitor thread error:{}", e);
						}
					}
				...
				logger.info(">>>>>>>>>>> sc-job, JobLosedMonitorHelper stop");

			}
		});
		monitorThread.setDaemon(true);
		monitorThread.setName("sc-job, admin JobLosedMonitorHelper");
		monitorThread.start();
	}
